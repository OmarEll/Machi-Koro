...
DeluxeExpansion::DeluxeExpansion(Collection_deluxe &col) {
    vector<Player*> Gamer;
    int nbJoueurs = 0;
...
void DeluxeExpansion::DoTurn(Player &current_player) {
    // Variables
    int dice = 0;
    string choice;
    // Fonction
                ...
                if (rcard.second.top()->getColor() == RED){
                    ...
                    while (!te.empty()){
                        ...
                        if (red_card->activate(dice) && !red_card->getRenovation()){
                            if (red_card->getCardName_Enum() != FrenchRestaurant && red_card->getCardName_Enum() != MembersOnlyClub && red_card->getCardName_Enum() != SushiBar){
        ...
        for ( auto pair_cards : all_players->getHand()->getEstablishments()){
            ...
            while (!tet.empty()){
                ...
                if (current_player.getId() == all_players->getId() && cards->getColor() != RED && cards->activate(dice) && !cards->getRenovation()){
                    ...
                    && cards->getCardName_Enum() != MackerelBoat && cards->getCardName_Enum() != TunaBoat && cards->getCardName_Enum() != SushiBar && cards->getCardName_Enum() != FoodWarehouse){
    ...
    dynamic_cast<DeluxeBoard*>(board_Game)->fillBoard();

}

void DeluxeExpansion::Do_Game() {
    initGame();
    Player* test = nullptr;
...
int DeluxeExpansion::dice_turn(Player &current_player) {
    string choice;
